...
int main() {
    ...
    if (!file.good()) {
        std::cerr << "Error: Model file '" << model_path << "' not found!" << std::endl;
        return -1;
    }
    ...
    detector->setInputSize(d_image.size());

    // 4. Inference
    // We pass the GpuMat (d_image) directly.
    // The 'faces' output will be a CPU Mat because it's a list of coordinates (small data).
    // OpenCV handles the logic of keeping the image processing on GPU and returning results to CPU.
    cv::Mat faces;
    detector->detect(d_image, faces);

    std::cout << "Detection complete. Found " << faces.rows << " faces." << std::endl;
    ...
    for (int i = 0; i < faces.rows; i++) {
        ...
        std::cout << " - Face " << i << " [Conf: " << confidence << "] at (" << x << "," << y << ")" << std::endl;

        // Draw on Host Image
        cv::rectangle(h_image, cv::Rect(x, y, w, h), cv::Scalar(0, 255, 0), 2);

        cv::circle(h_image, cv::Point2f(right_eye_x, right_eye_y), 3, cv::Scalar(0, 0, 255), -1);
        cv::circle(h_image, cv::Point2f(left_eye_x, left_eye_y), 3, cv::Scalar(0, 0, 255), -1);
        cv::circle(h_image, cv::Point2f(nose_x, nose_y), 3, cv::Scalar(0, 0, 255), -1);
    }
